...
export default function Onboarding() {
  ...
  const [dividas, setDividas] = useState([
    { credor: "", saldo: "", parcela: "", taxaAA: "", atrasada: false },
  ]);
  ...
  const addDiv = () =>
    setDividas(v => [...v, { credor: "", saldo: "", parcela: "", taxaAA: "", atrasada: false }]);
  const rmDiv = i => setDividas(v => v.filter((_, idx) => idx !== i));

  const rendaN = +renda || 0,
    fixasN = +fixas || 0,
    varsN = +variaveis || 0;
  ...
  return (
    <main className="mx-auto max-w-3xl p-4">
      ...
      {step === 1 && (
        <section className="mt-4 space-y-3">
          <label className="text-sm text-white/80">
            Renda mensal líquida <Info tip={TIPS.renda} />
          </label>
          <Currency value={renda} onChange={setRenda} />

          <label className="text-sm text-white/80">
            Despesas fixas <Info tip={TIPS.fixas} />
          </label>
          <Currency value={fixas} onChange={setFixas} />

          <label className="text-sm text-white/80">
            Despesas variáveis <Info tip={TIPS.variaveis} />
          </label>
          <Currency value={variaveis} onChange={setVariaveis} />

          <label className="text-sm text-white/80">
            Caixa hoje <Info tip={TIPS.caixa} />
          </label>
      ...
      {step === 2 && (
        <section className="mt-4 space-y-3">
          <div className="text-sm text-white/80">
            Dívidas <Info tip={TIPS.qtdDividas} />
          </div>

          {dividas.map((d, i) => (
            <div
              key={i}
              className="grid grid-cols-1 gap-2 rounded-xl bg-[var(--s2)] p-3 sm:grid-cols-5"
            >
              <Input
                placeholder="Credor"
                value={d.credor}
              ...
              <label className="inline-flex items-center gap-2 text-sm">
                <input
                  ...
                  onChange={e => {
                    const cp = [...dividas];
                    cp[i].atrasada = e.target.checked;
                    setDividas(cp);
                  }}
                />
                Atrasada
              ...
              <button onClick={() => rmDiv(i)} className="btn text-xs">
                Remover
              </button>
            </div>
          ))}

          <button onClick={addDiv} className="btn text-sm">
            + Adicionar dívida
          </button>
        </section>
      )}
      ...
      {step === 3 && (
        <section className="mt-4 space-y-3">
          <label className="text-sm text-white/80">
            Reserva alvo <Info tip={TIPS.reserva} />
          </label>
          <select
            className="mt-1 w-full rounded-xl bg-[var(--s2)] p-2"
            value={reserva}
            onChange={e => setReserva(e.target.value)}
          >
            <option value="0.5m">0,5 mês</option>
            <option value="1m">1 mês</option>
            <option value="3m">3 meses</option>
          </select>

          <label className="text-sm text-white/80">
            Preferência de método <Info tip={TIPS.metodo} />
          </label>
          <select
            className="mt-1 w-full rounded-xl bg-[var(--s2)] p-2"
            value={prefer}
          ...
          <p className="mt-1 text-xs text-white/60">{METHOD_INFO[metodo] || ""}</p>

          <label className="mt-3 text-sm text-white/80">
            Como você se descreve <Info tip={TIPS.comportamento} />
          </label>
          <select
            className="mt-1 w-full rounded-xl bg-[var(--s2)] p-2"
            value={beh}
          ...
          <p className="mt-1 text-xs text-white/60">{BEHAVIOR_INFO[beh] || ""}</p>
